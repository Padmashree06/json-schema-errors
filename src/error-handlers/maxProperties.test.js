...
describe("maxProperties error handler", async () => {
  ...
  test("maxProperties fail", async () => {
    ...
    expect(errors).to.eql([
      {
        message: localization.getMaxPropertiesErrorMessage(1),
        instanceLocation: "#",
        schemaLocations: [`${schemaUri}#/maxProperties`]
      }
    ]);
  });

  test("maxProperties pass", async () => {
    registerSchema({
      $schema: "https://json-schema.org/draft/2020-12/schema",
      maxProperties: 1
    }, schemaUri);
